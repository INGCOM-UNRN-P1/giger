---
title: "Guía de Extensión y Creación de Plugins: giger"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de giger"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-giger-plugins)=
# Guía de Extensión y Plugins: giger

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`giger`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-giger-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`giger` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para giger`.
- **Punto de Entrada Oficial**: `giger.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-giger-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para giger."""
    name = "custom_rule"
...
Para que `giger` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."giger.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `giger` la reconozca:
...
(manual-giger-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `giger` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = giger.ejecutar_analisis(
...
(manual-giger-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `giger` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar giger
        run: pip install -e ./giger
        
      - name: Ejecutar Auditoría
        run: giger check src/ include/ --json > reporte.json
````

---

(manual-giger-plugins-ejercicios)=
